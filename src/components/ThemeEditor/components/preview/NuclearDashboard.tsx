...
const NuclearDashboard: React.FC<NuclearDashboardProps> = ({
  colorKey,
  themeValues,
}) => {
  ...
  const maintenanceTasks = [
    { id: 'MT-1', task: 'Cooling System Inspection', assignee: 'Engineering Team A', status: 'In Progress', due: '03/26/2025' },
    { id: 'MT-2', task: 'Containment Valve Replacement', assignee: 'Maintenance Team C', status: 'Scheduled', due: '03/29/2025' },
    { id: 'MT-3', task: 'Radiation Monitor Calibration', assignee: 'Safety Team B', status: 'Completed', due: '03/23/2025' },
    { id: 'MT-4', task: 'Control Rod Mechanism Test', assignee: 'Engineering Team D', status: 'Pending Approval', due: '04/02/2025' },
    { id: 'MT-5', task: 'Emergency Generator Testing', assignee: 'Electrical Team A', status: 'Scheduled', due: '03/31/2025' },
  ];

  // Shift schedule
  const shifts = [
    { name: 'Morning', time: '6:00 - 14:00', supervisor: 'Sarah Chen', staff: 12, color: 'green' },
    { name: 'Afternoon', time: '14:00 - 22:00', supervisor: 'Michael Rodriguez', staff: 11, color: 'blue' },
      ...
      >
        ...
        <HStack spacing={6}>
          ...
          <Box>
            ...
            <HStack>
              ...
              <Badge colorScheme="blue">
                Shift: {currentDate.getHours() >= 6 && currentDate.getHours() < 14 ? 'Morning' : 
                      currentDate.getHours() >= 14 && currentDate.getHours() < 22 ? 'Afternoon' : 'Night'}
              </Badge>
            </HStack>
          </Box>
          ...
          <HStack>
            ...
            <IconButton aria-label="Settings" icon={<SettingsIcon />} colorScheme={colorKey} variant="ghost" />
            <IconButton aria-label="Logout" icon={<FaSignOutAlt />} colorScheme={colorKey} variant="ghost" />
          </HStack>
      ...
      <Grid templateColumns="repeat(12, 1fr)" gap={6}>
        ...
        <GridItem colSpan={{ base: 12, lg: 9 }}>
          <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={4} mb={6}>
            {/* Power Output */}
            <Card boxShadow="md" bgGradient={`linear(to-r, ${colorKey}.500, ${colorKey}.600)`} color="white">
              <CardBody p={4}>
                <Flex justify="space-between" align="center">
                  <Box>
                    <Text fontWeight="medium" mb={1}>Reactor Power</Text>
                    <Heading size="xl">{reactorPower}%</Heading>
                    <Text fontSize="sm">of Maximum Capacity</Text>
                  </Box>
                  <Box>
                    <CircularProgress value={reactorPower} color="white" thickness="12px" size="80px">
            ...
            <Card boxShadow="md">
              <CardBody p={4}>
                ...
                <Flex justify="space-between" align="center">
                  ...
                  <Box textAlign="right">
                    <Text fontSize="sm" fontWeight="medium">Safe Range</Text>
          ...
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4} mb={6}>
            ...
            <Card boxShadow="sm">
              <CardBody p={4}>
                <HStack justify="space-between" mb={1}>
                  ...
                  <Badge 
                    colorScheme={radiationLevel < 0.10 ? "green" : "orange"}
                    variant="subtle"
                  >
                    {radiationLevel < 0.10 ? "Normal" : "Elevated"}
                  </Badge>
                </HStack>
                <Heading size="lg">{radiationLevel} mSv/h</Heading>
                <Text fontSize="xs" mt={1}>Regulatory limit: 0.25 mSv/h</Text>
              </CardBody>
            </Card>
            ...
            <Card boxShadow="sm">
              <CardBody p={4}>
                ...
                <Text fontSize="xs" mt={1}>Target efficiency: 97.0%</Text>
              </CardBody>
            </Card>
            ...
            <Card boxShadow="sm">
              <CardBody p={4}>
                ...
                <Text fontSize="xs" mt={1}>Capacity: 1100 MW</Text>
              </CardBody>
            </Card>
          </SimpleGrid>
                        ...
                        <Td>
                          <Badge 
                            colorScheme={
                              ...
                              task.status === 'Pending Approval' ? 'orange' : 'gray'
                            }
                          >
                            {task.status}
                          </Badge>
        ...
        <GridItem colSpan={{ base: 12, lg: 3 }}>
          ...
          <Card boxShadow="md" mb={6} borderWidth="1px" borderColor={borderColor}>
              ...
              <VStack divider={<Divider />} spacing={0} align="stretch">
                {alerts.map((alert, index) => (
                  <Box 
                    ...
                    p={3} 
                    bg={
                      alert.level === 'critical' ? 'red.50' :
                      alert.level === 'warning' ? 'orange.50' :
                      alert.level === 'info' ? 'blue.50' : 'green.50'
                    }
                  >
                    <Flex justify="space-between" mb={1}>
                      <HStack>
                        <Icon 
                          as={
                            alert.level === 'critical' ? FaExclamationCircle :
                            alert.level === 'warning' ? FaExclamationTriangle :
                            alert.level === 'info' ? InfoIcon : CheckIcon
                          } 
                          color={
                            alert.level === 'critical' ? 'red.500' :
                            alert.level === 'warning' ? 'orange.500' :
                            alert.level === 'info' ? 'blue.500' : 'green.500'
                          }
                        />
                        <Text 
                          fontWeight="bold" 
            ...
            <CardFooter p={3} borderTop="1px solid" borderColor={borderColor} justifyContent="space-between">
              <Button size="xs" variant="ghost" colorScheme={colorKey}>
                Mark All Read
              </Button>
              <Button size="xs" variant="ghost" colorScheme={colorKey} rightIcon={<ChevronRightIcon />}>
                View All
              </Button>
            </CardFooter>
          </Card>
            ...
            <CardBody p={3}>
              {shifts.map((shift, index) => (
                ...
                >
                  <Box flex="1">
                    <Flex justify="space-between" align="center">
                      <Text fontWeight="bold" fontSize="sm">{shift.name} Shift</Text>
                      <Badge colorScheme={shift.color} size="sm">
                        {shift.time}
                      </Badge>
                    </Flex>
                    <Text fontSize="xs" mt={1}>Supervisor: {shift.supervisor}</Text>
                ...
                {safetyMetrics.map((metric, index) => (
                  ...
                  >
                    <Flex justify="space-between" align="center">
                      ...
                      <Flex align="center">
                        <Text fontWeight="bold" mr={1}>{metric.value}</Text>
                        {metric.change !== 'same' && (
                          <Icon 
                            as={metric.change === 'up' ? ArrowUpIcon : ArrowDownIcon} 
                            color={
                              (metric.metric.includes('Incident') || metric.metric.includes('Issues')) 
                                ? (metric.change === 'up' ? 'red.500' : 'green.500')
                                : (metric.change === 'up' ? 'green.500' : 'red.500')
                            }
                            boxSize={3}
                          />
                        )}
